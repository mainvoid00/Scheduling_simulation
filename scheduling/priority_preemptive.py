...
class PriorityPreemptive(Scheduling):
    ...
    def Run(self,):
        ...
        while flag:
            ...
            if self.DISPATCH:
                ...
                if self.ready_queue and self.process[self.ready_queue[0]].priority < self.process[dispatch_pid].priority:
                    ...
                    print("------------------------------")
                    print("PID", self.process[dispatch_pid].PID, "enqueue now time =", run_time)
                    ...
                    if(len(self.ready_queue) == 0):
                        ...
                        for i in range(len(self.ready_queue)):
                            if self.process[self.ready_queue[i]].priority > self.process[dispatch_pid].priority:
                ...
                if int((run_time - dispatch_start_time) == self.remaining_burst_time[dispatch_pid]) :
                    print("------------------------------")
                    print("terminate PID", dispatch_pid, "now time = ",run_time)
                    print("------------------------------")
                    self.process[dispatch_pid].burst_end_time.append(run_time)
                    self.remaining_burst_time[dispatch_pid] -=(run_time - dispatch_start_time)
